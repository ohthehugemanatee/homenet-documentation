...
- `gatus/config.yaml` — Gatus external health monitor config: probes the kube-apiserver VIP every minute and Pushover-alerts on 3+ consecutive failures. Credentials injected from `/etc/ansible/pushover.env` via docker-compose `env_file`.
