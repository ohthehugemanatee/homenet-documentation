...
- `workflows/lint.yaml` — the repo-wide linters, gated Trivy scans, and advisory kube-score/polaris checks. Its `github-scripts` job runs the `scripts/tests/` unit tests, Python (`unittest`) and JavaScript (`node --test`), and enforces the one-producer rule below.
