...
- `workflows/lint.yaml` — `yamllint` + `kubeconform` + `kustomize build` + `ansible-lint` (+ advisory kube-score/polaris/trivy). Its `github-scripts` job also runs the `scripts/tests/` unit tests, both Python (`unittest`) and JavaScript (`node --test`).
...
`pull_request_template.md` provides the checklist for human reviewers: Part 1 is the standard code review checklist; Part 2 is the mandatory AI-specific validation checklist (omission, logic, dependency, context, drift, comment budget); Part 3 is the explicit security & privacy checklist (secrets, RBAC, network exposure, container security, privacy). The AI review workflow (`pr-review.yaml`) is prompted to run the same Part 2 and Part 3 checks and must report each item explicitly. Recurring AI mistakes found during review belong in `agentic-review-exceptions.yaml` with rationale.
