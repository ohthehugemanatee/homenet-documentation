...
- `workflows/lint.yaml` — `yamllint` + `kubeconform` + `kustomize build` + `ansible-lint` (+ advisory kube-score/polaris/trivy). Its `github-scripts` job also runs the `scripts/tests/` unit tests, Python (`unittest`) and JavaScript (`node --test`) alike.
