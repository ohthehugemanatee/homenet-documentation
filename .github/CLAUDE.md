...
- `workflows/autofix.yaml` — fires on `lint.yaml` / `test-cluster.yaml` failure; runs `scripts/autofix.py`. That script tries a deterministic pre-pass first, keyed off the failing job's name in `FIXERS`, and only calls the model when the pre-pass produces nothing. The model half is an agentic loop: read_file / write_file / run_bash; commits + comments.
