...
- **A config change carries its doc fix.** When a change touches K8s manifests, Helm values, Ansible vars or playbooks, CI workflows or docker-compose, check the `*.md` in the affected directory and the repo root: text the change made factually wrong is a bug, and the fix ships in the same PR. It does not mean the topic could carry more explanation, more operational detail, or a note on how the bug was found; that goes in the PR description if anywhere.
