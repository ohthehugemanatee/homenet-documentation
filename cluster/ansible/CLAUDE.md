...
- **`deliver-tls-certs.yaml`** — delivers cluster-issued certificates to the hosts in the `tls_clients` inventory group (ADR-0007); `serial: 1`, idempotent, safe to re-run. Each host carries `tls_deliveries`, a list of deliveries mapped onto the `tls_cert` role. It loads **no `vault_file`**: after part 2 of #9 Ansible reads its secrets from OpenBao, and a job that needed OpenBao could not be the thing that fixes OpenBao when an expired certificate has taken it down. Its Pushover credentials come from `/etc/ansible/pushover.env` instead, which `shoebox/shoebox-ansible-setup.yaml` writes outside the vault. One delivery per host: the role registers its copy results as host facts that outlive the loop iteration.
...
Delivers a named TLS `Secret` to an off-cluster host: reads it with `kubernetes.core.k8s_info` (ADR-0005 contract), writes `tls_cert_format: separate` (cert + key files, the shoebox reverse proxy) or `combined` (one PEM, fullchain + key, Pi-hole v6) to the caller's destination, and runs `tls_cert_reload_command` only when `ansible.builtin.copy` reports a change. `community.crypto.x509_certificate_info` reads the delivered cert back and fails the run when `notAfter` is inside `tls_cert_min_days`, so a stalled delivery job is loud rather than silent. `deliver-tls-certs.yaml` is the playbook that calls it.
...
- **Pushover alerts:** `WARNING` when an auto-rebuild succeeded; `CRITICAL` for unrecovered failures (CP failure, agent rebuild failed, agent failure before `k3s_health` ran). New rescue paths follow this severity contract. The upgrade paths read `pushover_app_token` / `pushover_user_key` from `vault_file`; `deliver-tls-certs.yaml` reads `/etc/ansible/pushover.env` instead, for the reason in its contract above.
