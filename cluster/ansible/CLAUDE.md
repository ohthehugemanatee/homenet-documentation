...
- **`longhorn-preflight.yaml`** — read-only pre-upgrade gate for Longhorn (#280). Operates on cluster CRs, not node OS (ADR-0005). Exits non-zero on a faulted `Volume` or a `BackingImage` with a failed disk file; reports degraded, mid-transition and engine-lagging volumes without blocking. Robustness is read only on **attached** volumes — a detached one reports `unknown` because no engine is running, and treating that as a defect leaves the gate permanently red. Run before every hop of the version chain, not once.
