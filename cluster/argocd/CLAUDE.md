...
- **Auto-sync** (prune + self-heal): stable media and utility apps. These get `metadata.finalizers: [resources-finalizer.argocd.argoproj.io]` so removing the Application YAML cascade-deletes the resources. `traefik-default-tls` is the exception: auto-sync for self-heal, but no finalizer and `prune: false`, because the Certificate it holds must survive both (#391).
