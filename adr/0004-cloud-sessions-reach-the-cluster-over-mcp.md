...
Sessions talk to an in-cluster `kubernetes-mcp-server` over ordinary outbound HTTPS via
its own Cloudflare hostname, with Access checking a Service Token at the edge (#145).
...
- `K8S_API_HOSTNAME` is needed only to rebuild the manual forwarder by hand.
- Session reach is bounded by what the MCP server exposes plus the `view` ClusterRole
  bound to `claude-remote-debug` — not by whatever a kubeconfig happens to allow.
